...
from checkout_sdk.oauth_access_token import OAuthAccessToken
...
class OAuthSdkCredentials(SdkCredentials, metaclass=ABCMeta):
    ...
    def init(http_client: Session, environment: Environment, client_id, client_secret, authorization_uri, scopes: list):
        credentials = OAuthSdkCredentials(http_client, environment, client_id, client_secret, authorization_uri,
                                          scopes)
    ...
    def get_authorization(self, authorization_type: AuthorizationType):
        if authorization_type in (
            ...
            return SdkAuthorization(PlatformType.OAUTH, self.get_access_token().token)
