...
class Validator:
    ...
    def validate_and_set_source_type(cls, source):
        source_type_switch = {
            ...
            'customer': lambda: 'email' in source or ('id' in source and type(source['id']) is str and source['id'].startswith('cus_')),
            'token': lambda: 'token' in source,
            'id': lambda: 'id' in source and type(source['id']) is str and source['id'].startswith('src_')
        }
