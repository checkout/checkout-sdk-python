...
def test_should_increment_payment_authorization(default_api):
    payment_response = make_authorization_estimated_payment(default_api)

    authorization_request = AuthorizationRequest()
    authorization_request.amount = 5
    authorization_request.reference = new_uuid()

    void_response = default_api.payments.increment_payment_authorization(payment_response.id, authorization_request)
...
def test_should_increment_payment_authorization_idempotently(default_api):
    payment_response = make_authorization_estimated_payment(default_api)
    ...
    increment_response_1 = default_api.payments.increment_payment_authorization(payment_response.id,
                                                                                authorization_request, idempotency_key)
    assert_response(increment_response_1)

    increment_response_2 = default_api.payments.increment_payment_authorization(payment_response.id,
                                                                                authorization_request, idempotency_key)
...
def make_authorization_estimated_payment(default_api: CheckoutApi):
    ...
    payment_response = default_api.payments.request_payment(payment_request, None)
