...
from checkout_sdk.payments.payments import RequestCardSource, PaymentRequest, RequestTokenSource, ThreeDsRequest
...
def make_token_payment(default_api: CheckoutApi):
    ...
    payment_response = default_api.payments.request_payment(payment_request)
    assert_response(payment_response, 'id')
    return payment_response


def make_3ds_card_payment(default_api: CheckoutApi, attempt_n3d: bool = False):
    request_card_source = RequestCardSource()
    request_card_source.number = VisaCard.number
    request_card_source.expiry_month = VisaCard.expiry_month
    request_card_source.expiry_year = VisaCard.expiry_year
