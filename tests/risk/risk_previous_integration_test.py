...
from checkout_sdk.instruments.instruments_previous import InstrumentAccountHolder, CreateInstrumentRequest
...
def test_should_pre_capture_and_authenticate_card(previous_api):
    ...
    authentication_assessment_request(previous_api, card_source)
    pre_capture_assessment_request(previous_api, card_source)


@pytest.mark.skip(reason='not available')
def test_should_pre_capture_and_authenticate_customer(previous_api):
    customer = CustomerRequest()
    customer.email = random_email()
    customer.name = 'Customer'
    customer.phone = phone()

    customer_response = previous_api.customers.create(customer)

    source_prism = CustomerSourcePrism()
    source_prism.id = customer_response.id

    authentication_assessment_request(previous_api, source_prism)
    pre_capture_assessment_request(previous_api, source_prism)


@pytest.mark.skip(reason='not available')
def test_should_pre_capture_and_authenticate_id(previous_api):
    ...
    instrument_response = previous_api.instruments.create(instrument_request)
...
@pytest.mark.skip(reason='not available')
def test_should_pre_capture_and_authenticate_token(previous_api):
    card_token_request = CardTokenRequest()
    card_token_request.number = VisaCard.number
    card_token_request.expiry_month = VisaCard.expiry_month
    card_token_request.expiry_year = VisaCard.expiry_year
    ...
    authentication_assessment_request(previous_api, token_source)
    pre_capture_assessment_request(previous_api, token_source)
