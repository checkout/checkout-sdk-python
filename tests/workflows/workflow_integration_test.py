...
def test__should_create_and_get_workflows(four_api):
    ...
    for condition in workflow_response['conditions']:
        ...
        if condition['type'] == 'event':
            assert 'events' in condition and condition['events'].__len__() > 0
        if condition['type'] == 'entity':
            assert 'entities' in condition and condition['entities'].__len__() > 0
        if condition['type'] == 'processing_channel':
            assert 'processing_channels' in condition and condition['processing_channels'].__len__() > 0
