...
def test_should_retrieve_event_types(default_api):
    all_event_types = default_api.events.retrieve_all_event_types()
    assert_response(all_event_types, 'items')
    assert all_event_types.items.__len__() == 2

    version_one_items = default_api.events.retrieve_all_event_types(all_event_types.items[0].version)
    ...
    version_two_items = default_api.events.retrieve_all_event_types(all_event_types.items[1].version)
    assert_response(version_one_items, 'items')
...
def test_should_retrieve_events_by_payment_id_and_retrieve_event_by_id_and_get_notification(default_api):
    ...
    events = retriable(callback=default_api.events.retrieve_events,
    ...
    retrieve_event = default_api.events.retrieve_event(events.data[0].id)

    assert_response(retrieve_event, 'id', 'notifications', 'data', 'type', 'version')

    event_notification = default_api.events.retrieve_event_notification(retrieve_event.id,
