...
def test_should_query_disputes(previous_api):
    ...
    response = previous_api.disputes.query(query)
...
def test_should_upload_file(previous_api):
    ...
    response = previous_api.disputes.upload_file(request)
    assert_response(response, 'id', '_links')

    file_details = previous_api.disputes.get_file_details(response.id)
...
def test_should_test_full_disputes_workflow(previous_api):
    payment = make_card_payment(previous_api, amount=1040, capture=True)

    query = DisputesQueryFilter()
    query.payment_id = payment.id

    query_response = retriable(callback=previous_api.disputes.query,
    ...
    upload_file_response = previous_api.disputes.upload_file(request)
    ...
    put_response = previous_api.disputes.put_evidence(dispute_id, evidence_request)
    assert_response(put_response, 'http_metadata')

    evidence = previous_api.disputes.get_evidence(dispute_id)
