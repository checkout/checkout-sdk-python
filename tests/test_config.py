...
class ConfigTests(CheckoutSdkTestCase):
    ...
    def test_os_env_key_with_sandbox_as_default(self):
        config = Config()
        self.assertEqual(config.secret_key, os.environ['CKO_SECRET_KEY'])
        self.assertEqual(config.public_key, os.environ['CKO_PUBLIC_KEY'])
    ...
    def test_public_key_in_constructor_overrides_os_env(self):
        self.assertRaises(Exception, Config, public_key='pk_invalid')